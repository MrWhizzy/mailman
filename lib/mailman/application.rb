  ...
  class Application
    ...
    def run
      ...
      # IMAP
        ...
        imap = Receiver::IMAP.new(options)
        options[:idle] ? idle_loop(imap) : polling_loop(imap)
      ...
      # Maildir
        ...
        if config.watch_maildir
          ...
          callback = proc do |_modified, added, _removed|
      ...
      loop do
        begin
          ...
          unless (tries -= 1).zero?
            ...
            rescue StandardError # don't crash in the crash handler
            end
            retry
          end
        ensure
